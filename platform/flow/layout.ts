...
import { COL_GAP, ROW_GAP, TITLE_H, nodeBox, type NodeBox } from './geometry'
...
export interface FlowLayout {
  ...
  isGrid: boolean
  /** The node box this layout was built on. Carried on the layout so the edges
   *  and the canvas read the same geometry rather than importing a constant
   *  that is only right for one device. */
  box: NodeBox
}
...
export function computeLayout(screens: ScreenDef[], box: NodeBox = nodeBox()): FlowLayout {
  const byId = new Map(screens.map((s) => [s.id, s] as const))
  const links = resolveLinks(screens)

  if (links.length === 0) {
    const nodes = gridLayout(screens, box)
    return { nodes, links, ...bounds(nodes, box), isGrid: true, box }
  }
  ...
  return { nodes, links, ...bounds(nodes, box), isGrid: false, box }
}
