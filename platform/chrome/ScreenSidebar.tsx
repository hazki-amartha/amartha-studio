...
const ROW_ACTIVE = 'truncate rounded-8 bg-primary-50 px-12 py-8 text-left text-14 font-bold text-link'
const ROW_IDLE =
  'truncate rounded-8 px-12 py-8 text-left text-14 text-default hover:bg-neutral-50'
...
export function ScreenSidebar({ project }: { project: ProjectIndexEntry }) {
  ...
  return (
    <div className="flex flex-col gap-16">
      <Link
        href="/"
        className="flex items-center gap-4 px-12 text-12 text-caption hover:text-default"
      >
        <ChevronLeftIcon className="size-12 shrink-0" />
        All projects
      </Link>

      <p className="truncate px-12 text-16 font-bold text-default">{project.name}</p>

      <nav aria-label="Screens" className="flex flex-col gap-2">
        <p className="px-12 py-4 text-10 font-bold uppercase text-caption">Screens</p>
