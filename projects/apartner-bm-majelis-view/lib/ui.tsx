...
// the header. That 80 is the sum of two tokens (h-32 + h-48), not a spacing
// token of its own, so it rides in an inline `top` — the one place the system
// already allows a computed layout value (see `Meter`'s data-driven width). The
// blue bleed (`-mx-16` + `bg-canvas-blue`) masks the cards scrolling under it
// back to the canvas colour.
const STEP_STICKY_TOP = 32 + 48

// The title has to sit ABOVE the cards it pins over (so a meter or a card edge
// can't show through it) but BELOW the pinned header (so it tucks under the bar
// rather than riding on top of it as it approaches). The Screen's chrome is
// `z-10`, and the cards are unpositioned, so the title takes a z BETWEEN them:
// positive, to clear the cards, and under 10, to stay behind the header. A
// value, not a token — the same layering constant the platform's z-10 is — so
// it rides inline beside `top`.
const STEP_STICKY_Z = 5
...
export function StepSectionTitle({
  ...
  children: ReactNode
  /** A quiet counter pinned to the far right — the section's place in the run
   *  ("2 dari 8"). Grey and regular, so it reports position without competing
   *  with the name. */
  trailing?: ReactNode
}) {
  return (
    <h2
      style={{ top: STEP_STICKY_TOP, zIndex: STEP_STICKY_Z }}
      className="sticky -mx-16 flex items-center gap-8 bg-canvas-blue px-16 pb-8 pt-16 text-18 font-bold text-default"
    >
      {children}
      {trailing != null ? (
        <span className="ml-auto shrink-0 text-14 font-regular text-caption">{trailing}</span>
      ) : null}
    </h2>
  )
}
