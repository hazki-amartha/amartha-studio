...
import { SELF, assigneeName } from '../lib/pipeline'
...
export function PoiNewScreen() {
  ...
  const [sheet, setSheet] = useState<'assignee' | null>(null)

  const pinned = Boolean(coord)
  const ready = name.trim() !== '' && area.trim() !== ''

  function save() {
    if (!ready) return
    pipelineStore.addPoi({ name, area, mapsCoord: coord, assignedTo, contactName, contactPhone, photo, note })
    flow.go('poi-detail')
  }
