...
// Tagih Pembayaran — the collect menu, shared by the majelis visit (collect.tsx)
// and the home visit's Tagih step (home-visit.tsx): one question, asked one way.
//
...
// options the BP APP 2026 Figma draws — Bayar penuh, Pelunasan dini, Bayar
// jumlah lain, Tidak bayar.
//
...
import { BottomSheet, Button, InputNominal } from '@/design-system/components'
import { outstandingBalanceOf, outstandingOf, rupiah, type Mitra } from './data'
import { Camera, CheckCircleFill, Image as ImageIcon } from '@/design-system/icons'
import { paidOf, paidViaPoket, store, useApp } from './store'
import { AngsuranCard, JanjiBayarCard } from './mitra-card'
import {
  ChoiceList,
  DateWheel,
  NominalCard,
 ...
 */
type Pick = 'lunas' | 'dini' | 'sebagian' | 'poket' | 'tanggung' | 'tidak'
...
export function CollectPanel({
  ...
  header?: ReactNode
  /** The janji bayar on file; defaults to the mitra's own. */
  janjiDate?: string
  onSeeAll: () => void
  ...
  const s = useApp()
  const janji = janjiDate ?? mitra.ptp

  const owed = outstandingOf(mitra)
  // Pelunasan dini settles the whole loan today: everything still owed on it,
  // not this week's bill.
  const payoff = outstandingBalanceOf(mitra)
  ...
  const recorded: Pick | null = refusal
    ? 'tidak'
    : s.payMode[mitra.id] === 'dini' && existing > 0
      ? 'dini'
      : settledByGroup
        ? 'tanggung'
        : settledViaPoket
  ...
  const [sheet, setSheet] = useState<Mode | null>(
    // A pelunasan dini has no sheet — like "Bayar penuh" it records straight
    // from the page — so reopening one lands on the page with it picked.
    recorded === null || recorded === 'dini'
      ? null
      : recorded === 'lunas' || recorded === 'sebagian'
        ? 'bayar'
        : recorded,
  )
  ...
  // Each reason is a pick plus, for "Lainnya", her own words — stored joined
  // as "Lainnya: …" and split back apart when a recorded outcome is reopened.
  const onFileRefusal = splitOther(refusal?.reason)
  const onFileShortfall = splitOther(s.shortfallReasons[mitra.id])
  const [reason, setReason] = useState<string | null>(onFileRefusal.pick)
  const [reasonText, setReasonText] = useState(onFileRefusal.text)
  const [shortfall, setShortfall] = useState<string | null>(onFileShortfall.pick)
  const [shortfallText, setShortfallText] = useState(onFileShortfall.text)
  ...
  // The receipt camera, over the sheet, for "sudah bayar sendiri".
  const [camera, setCamera] = useState(false)
  const [deathDate, setDeathDate] = useState<DateParts>(
    parseDateLabel(refusal?.deathDate) ?? { day: 21, month: 7, year: 2026 },
  )
  ...
  // "Sudah bayar sendiri" leaves nothing to chase, so it has no promise to ask.
  //
  // Step 2 is the janji bayar — except on two refusals: "sudah bayar sendiri"
  // leaves nothing to chase (so no step 2), and "meninggal dunia" asks for the
  // date she died instead of a date to pay.
  const second: 'ptp' | 'date' | null =
    sheet === 'tidak'
      ...
      : short
        ? 'ptp'
        : null
  const hasPtpStep = second === 'ptp'

  // What step 1 has to carry before "Lanjut" (or, with no step 2, the save)
  // unlocks.
  const step1Done =
    sheet === 'tidak'
      ? // A claim she already paid is backed by a photo of the receipt.
        reasonDone(reason, reasonText) && (reason !== PAID_ELSEWHERE || proof)
      : sheet === 'bayar'
        ? typed > 0 && (!short || reasonDone(shortfall, shortfallText))
        : sheet === 'poket'
          ? // Same rules as a cash payment — an amount, and a reason if it fell
            ...
            typed > 0 && (!short || reasonDone(shortfall, shortfallText)) && proof
  ...
  const onLastStep = second === null || step === 2
  const canAdvance = step === 1 ? step1Done : second !== 'ptp' || ptp !== undefined
  ...
  /**
   * Lanjut. A "Bayar penuh" or "Pelunasan dini" needs nothing else said about it — the figure is the bill,
  ...
  function confirmPick() {
    ...
    if (pick === 'dini') {
      store.setPayMode(mitra.id, 'dini')
      store.collect(mitra, payoff)
      store.setPartialPtp(mitra.id, null)
      store.clearDropOut(mitra.id)
      onDone()
      return
  ...
  function save() {
    ...
    if (sheet === 'tidak') {
      ...
      store.setNonPayment(mitra, {
        reason: joinOther(reason, reasonText) as string,
        ptp: hasPtpStep ? (ptp ?? null) : null,
        deathDate: second === 'date' ? dateLabel(deathDate) : undefined,
      })
      store.setPoketProof(mitra.id, reason === PAID_ELSEWHERE && proof)
      store.clearDropOut(mitra.id)
    } else if (sheet === 'poket') {
      store.recordPoket(
        mitra,
        typed,
        short ? (joinOther(shortfall, shortfallText) as string) : undefined,
      )
  ...
  return (
    <>
      {/* What she has been paying and what she owes, as one white panel over
          the grey floor, its bottom corners rounded per the BP APP 2026 Figma:
          the week grid, the last instalment that landed, the bill, and the
          promise on file. */}
      <div className="-mx-16 -mt-16 flex flex-col gap-12 rounded-b-16 border-b border-default bg-neutral-white p-16">
        {header}
        <AngsuranCard mitra={mitra} flat onSeeAll={() => onSeeAll()} />
        ...
        {janji && mitra.ptpAmount !== undefined ? (
          <>
            <div className="border-t border-default" />
            <JanjiBayarCard mitra={mitra} date={janji} flat />
          </>
        ) : null}
      </div>

      {/* The lower level: the choice, on the grey floor under the panel. */}
      <div
        role="radiogroup"
        aria-label="Berapa yang dibayar"
        className="flex flex-1 flex-col gap-12 pb-16 pt-4"
      >
        <SectionTitle>Berapa uang tunai yang dibayar?</SectionTitle>
        ...
        <div className="flex flex-col gap-8">
          <PickRow
            ...
            selected={pick === 'lunas'}
            disabled={locked && pick !== 'lunas'}
            onSelect={() => !locked && setPick('lunas')}
          />
          <PickRow
            title="Pelunasan dini"
            description={rupiah(payoff)}
            selected={pick === 'dini'}
            disabled={locked && pick !== 'dini'}
            onSelect={() => !locked && setPick('dini')}
          />
          {/* The four options the BP APP 2026 Figma draws. "Bayar via Poket"
              and "Tanggung renteng" are gone from the menu: a mitra who paid
              herself is a "Tidak bayar" (its description says so). Their
              sheets stay below so an outcome already on file can still be
              reopened from its card. */}
          <PickRow
            title="Bayar jumlah lain"
            selected={pick === 'sebagian'}
            disabled={locked && pick !== 'sebagian'}
            onSelect={() => !locked && setPick('sebagian')}
          />
          <PickRow
            title="Tidak bayar"
            description="Tidak bisa bayar saat ini/sudah bayar sendiri."
            selected={pick === 'tidak'}
            disabled={locked && pick !== 'tidak'}
            onSelect={() => !locked && setPick('tidak')}
          />
        </div>
      </div>
      ...
      <StickyBar>
        <div className="flex gap-12">
          ...
          <Button
            ...
            className="flex-1"
            disabled={pick === null}
            onClick={locked ? onDone : confirmPick}
          >
            Lanjut
          </Button>
        </div>
      ...
      <BottomSheet
        ...
        onBack={step === 2 && second === 'ptp' ? () => setStep(1) : undefined}
        size="md"
        // Per the BP APP 2026 Figma: the sheet is titled by what was picked
        // ("Bayar jumlah lain") or the question ("Alasan tidak bayar", "Janji
        // bayar"), and moves on with "Lanjut" at every step.
        title={
          sheet === null
            ? undefined
            : step === 2
              ? second === 'date'
                ? 'Isi tanggal meninggal'
                : 'Janji bayar'
              : sheet === 'tidak'
                ? 'Alasan tidak bayar'
                : sheet === 'tanggung'
        ...
        // Batal beside Lanjut only on the refusal, as the Figma draws it; the
        // amount sheet has the one button.
        secondaryAction={
          sheet === 'tidak' && (step === 1 || second === 'date') ? (
            <Button
              size="lg"
              variant="outline"
              className="w-full"
              onClick={() => (step === 2 ? setStep(1) : closeSheet())}
            >
              Batal
            </Button>
          ) : undefined
      ...
      >
                ...
                typed > owed.total
                  ...
                  : typed === owed.total
                    ...
                    : typed === 0
                      ...
                      : `Sisa ${rupiah(owed.total - typed)} akan jadi tunggakan.`
              }
            />
            {short ? (
              <ChoiceList
                label="Alasan bayar sebagian"
                options={SHORTFALL_REASONS}
                value={shortfall ?? undefined}
                onPick={setShortfall}
                other={{ text: shortfallText, onText: setShortfallText }}
              />
            ) : null}
          </>
        ) : null}
        ...
        {sheet === 'tidak' && step === 1 ? (
          <ChoiceList
            ...
            onPick={setReason}
            other={{ text: reasonText, onText: setReasonText }}
          />
        ) : null}

        {/* "Sudah bayar sendiri": the receipt she shows on her phone, photographed. */}
        {sheet === 'tidak' && step === 1 && reason === PAID_ELSEWHERE ? (
          <div className="flex flex-col gap-8">
            <span className="text-12 text-caption">Bukti pembayaran</span>
            {proof ? (
              <button
                type="button"
              ...
              >
                <span className="flex h-64 w-48 shrink-0 items-center justify-center rounded-8 bg-neutral-200 text-neutral-500">
                  <ImageIcon size={20} />
                </span>
                <span className="flex min-w-0 flex-1 flex-col gap-2">
                  ...
                  <span className="truncate text-12 text-caption">
                    AOTW230116000001 · Rp100.000
                  </span>
                </span>
                <span className="shrink-0 text-12 font-bold text-primary-500">Ulangi</span>
              </button>
            ) : (
              <button
                type="button"
        ...
        {step === 2 && second === 'date' ? (
          <DateWheel value={deathDate} onChange={setDeathDate} />
        ) : null}
        {step === 2 && second === 'ptp' ? (
          <ChoiceList
            label="Pilih tanggal"
      ...
      {/* The camera, pointed at the receipt on her phone. No camera in a
          prototype, so the viewfinder shows the receipt it would capture. */}
      <BottomSheet open={camera} size="fullscreen" onClose={() => setCamera(false)}>
        <div className="flex flex-col gap-16">
          <div className="flex flex-col items-center gap-8 rounded-16 bg-neutral-white p-16">
            <span className="text-green-500">
              <CheckCircleFill size={24} />
            </span>
            <span className="text-14 font-bold text-green-500">Berhasil</span>
            <span className="text-14 text-default">Bayar angsuran</span>
            <span className="text-24 font-bold text-default">Rp100.000</span>
            <div className="flex w-full flex-col gap-8 border-t border-default pt-12 text-12">
              {[
                ['Kode transaksi', 'AOTW230116000001'],
                ['Tanggal', '20 Jul 2026, 15:50'],
                ['Penyedia jasa', 'Amartha'],
                ['Nama', mitra.name],
              ].map(([k, v]) => (
          ...
          <button
            ...
            className="flex h-64 w-64 items-center justify-center self-center rounded-full border-4 border-primary-200 bg-primary-500 text-neutral-white"
          >
            <Camera size={24} />
          </button>
        </div>
