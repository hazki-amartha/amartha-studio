...
// The register's two demo switches, off — every register state starts from a
// visit with an address and a connection.
const visitNormal = { addressMissing: false, offline: false } as const

export const registerFresh = () =>
  store.set({ attendance: freshAttendance(), absenceReasons: {}, ...visitNormal })

/** The kumpulan has no address on file — "Alamat tidak tersedia." */
export const registerNoAddress = () =>
  store.set({ attendance: freshAttendance(), absenceReasons: {}, ...visitNormal, addressMissing: true })

/** The connection drops mid-register — the "internet terputus" sheet. */
export const registerOffline = () =>
  store.set({ attendance: freshAttendance(), absenceReasons: {}, ...visitNormal, offline: true })
...
export const registerAlmost = () => {
  ...
  store.set({ attendance, absenceReasons: {}, ...visitNormal })
}
...
// outcomes.
const noOutcomes = { payMode: {}, poketProof: {}, dropOut: {}, partialPtp: {}, refunds: {} } as const
...
/**
 * The task came back Ditolak: AFin recorded an early payoff after the BP had
 * already collected, so two mitra paid twice and are owed money back. Every
 * outcome is on file; the two cards carry the refund alert.
 */
export const queueRefund = () => {
  queueDone()
  const [, , viaLain, viaPoket] = collectible
  store.set({
    refunds: {
      [viaLain.id]: { via: 'metode lain', amount: 50000 },
      [viaPoket.id]: { via: 'Poket', amount: 50000 },
    },
  })
}
...
/** No mitra in this majelis qualifies for an offer — "Belum ada penawaran". */
export const offersEmpty = () => store.set({ openMajelis: 'mawar', ...clearGrowth, offersEmpty: true })

/**
 * One mitra said yes last visit but it was not processed then, so her card
 * asks "Apakah mitra sudah diproses penawarannya?" instead of pitching again.
 */
export const offersCarried = () => {
  const [, carried] = growthMembers()
  store.set({ openMajelis: 'mawar', ...clearGrowth, growthCarried: [carried.id] })
}
...
export const offersMixed = () => {
  ...
  store.set({
    ...
    growthReasons: {
      [declined.id]: 'Sudah punya di tempat lain',
      [carried.id]: 'Gangguan internet',
    },
  })
}
...
export const offersAll = () => {
  ...
  growthMembers().forEach((m, i) => {
    ...
    if (i === 1) growthReasons[m.id] = 'Gangguan internet'
  })
  store.set({
    openMajelis: 'mawar',
    offersEmpty: false,
