...
export function KumpulanCard({ entry, missing }: { entry: MajelisEntry; missing?: boolean }) {
  // No address on file: the caption stays, the route button goes — there is
  // nowhere to route to.
  if (missing) {
    return (
      <div className="flex flex-col gap-4">
        <span className="text-12 font-bold text-default">Alamat</span>
        <span className="text-12 text-caption">Alamat tidak tersedia.</span>
      </div>
    )
  }
  return (
