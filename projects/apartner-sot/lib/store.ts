 ...
 */
export type PayMode = 'penuh' | 'dini' | 'sebagian' | 'tanggung' | 'poket' | 'tidak' | 'keluar'
...
export interface NonPayment {
  ...
  ptp: string | null
  /** "3 Maret 2023" — recorded when the reason is "Meninggal dunia". */
  deathDate?: string
}
...
export interface AppState {
  ...
  /** Demo switch: the kumpulan has no address on file ("Alamat tidak tersedia."). */
  addressMissing: boolean
  /** Demo switch: the connection dropped mid-visit — the "internet terputus" sheet. */
  offline: boolean
  /**
   * mitraId → money she paid through another channel before the BP's task was
   * sent, which the BP must now hand back. Set when a sent task is returned
   * (Ditolak) because AFin already recorded an early payoff. `via` names the
  ...
  offersEmpty: boolean
  /**
   * mitraIds whose offer was accepted at an earlier visit but not yet
   * processed. Their card asks "sudah diproses?" instead of pitching again.
   */
  growthCarried: string[]
...
export const store = {
  ...
  setGrowthResult(
    ...
    // A no carries its reason; a yes carried to next time carries why it
    // wasn't processed today ("Jelaskan alasannya").
    if (reason && (result === 'tidak' || followUp === 'lanjut')) growthReasons[mitraId] = reason
