...
import { Badge, BottomSheet, Button, Input } from '@/design-system/components'
...
/** Single-choice picker: the design system's BottomSheet over plain radio rows. */
  ...
  return (
    <div className="flex items-start gap-8 rounded-r-8 border-l-2 border-default bg-canvas-blue p-8">
      <span className="flex min-w-0 flex-1 flex-col gap-2">
        ...
        <span className="whitespace-pre-line break-words text-12 text-default">{value}</span>
      </span>
      {onEdit ? (
        <button
          type="button"
...
export function ChoiceList({
   ...
   * sheet whose TITLE already says what is being picked.
   */
  hideLabel?: boolean
  /**
   * A free-text answer behind one option (default "Lainnya"): picking it opens
   * a text box under the list for the reason in her own words.
   */
  other?: { text: string; onText: (text: string) => void; option?: string; placeholder?: string }
}) {
  // One style for every radio list in a sheet, per the BP APP 2026 Figma: a
  // radio at the left, the label beside it, no box around the row.
  const otherOption = other?.option ?? 'Lainnya'
  return (
    <div className="flex flex-col gap-8">
        ...
        {options.map((option) => {
          ...
          return (
            <RadioRow
              ...
              checked={text === value}
              onChange={() => onPick(text)}
            />
          )
        })}
      </div>
      {other && value === otherOption ? (
        <textarea
          autoFocus
          rows={3}
          value={other.text}
          onChange={(e) => other.onText(e.target.value)}
          placeholder={other.placeholder ?? 'Masukkan alasan lainnya'}
...
/**
 * One radio row, bare — the mark at the left, no box. The sheet radio style
 * from the BP APP 2026 Figma, and a drop-in for `SelectableCard` inside a
 * bottom sheet (it takes the same props and ignores the ones it doesn't need).
 */
export function RadioRow({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onChange}
      className="flex w-full items-center gap-12 py-8 text-left disabled:opacity-50"
    >
      <span
        className={`flex h-20 w-20 shrink-0 items-center justify-center rounded-full border-2 ${
...
// --- NominalCard -----------------------------------------------------------
// The amount on a payment sheet, per the BP APP 2026 Figma: a bordered card
// captioned "Nominal", the figure large, a pencil at its edge, and what the
// figure leaves behind under it. The figure IS the field — tapping it or the
// pencil puts the cursor in it.

export function NominalCard({
  value,
  onValueChange,
  helperText,
}: {
  ...
  const shown = digits ? `Rp${Number(digits).toLocaleString('id-ID')}` : ''
  return (
    <div className="flex flex-col gap-4 rounded-16 border border-default p-16">
      ...
      <div className="flex items-center gap-8">
        <input
          ...
          className="min-w-0 flex-1 bg-transparent text-24 font-bold text-default outline-none placeholder:text-disabled"
        />
        <button
          type="button"
      ...
      {helperText ? <span className="text-12 text-caption">{helperText}</span> : null}
    </div>
  )
}

// --- DateWheel -------------------------------------------------------------
// The month / day / year picker from the BP APP 2026 Figma ("Isi tanggal
// meninggal"): three columns, the picked value large and in brand colour at
// the centre, two neighbours either side fading out. Tapping a neighbour moves
// the wheel to it — the prototype's stand-in for a flick.
...
/** "3 Maret 2023". */
export function dateLabel(d: DateParts): string {
  return `${d.day} ${MONTHS[d.month - 1]} ${d.year}`
}

export function parseDateLabel(label: string | undefined): DateParts | null {
  if (!label) return null
  const [day, month, year] = label.split(' ')
  const m = MONTHS.indexOf(month)
  return m < 0 ? null : { day: Number(day), month: m + 1, year: Number(year) }
}
  ...
  return (
    <div className="flex flex-1 flex-col items-center">
      {[-2, -1, 0, 1, 2].map((offset) => {
        ...
        const tone =
          offset === 0
            ? 'text-20 font-bold text-primary-500'
            : Math.abs(offset) === 1
              ? 'text-16 text-caption'
              : 'text-16 text-disabled'
        return (
          <button
            key={offset}
            type="button"
            disabled={item === undefined || offset === 0}
            onClick={() => onPick(i)}
            className={`flex h-40 w-full items-center justify-center ${tone}`}
          >
            {item ?? ''}
          </button>
        )
...
export function DateWheel({
  ...
  return (
    <div className="flex items-center" role="group" aria-label={`Tanggal: ${dateLabel(value)}`}>
      <WheelColumn items={MONTHS} index={value.month - 1} onPick={(i) => set({ month: i + 1 })} />
      <WheelColumn
        items={Array.from({ length: days }, (_, i) => String(i + 1))}
        index={value.day - 1}
        onPick={(i) => set({ day: i + 1 })}
      />
      <WheelColumn items={years} index={value.year - fromYear} onPick={(i) => set({ year: fromYear + i })} />
    </div>
  )
}
