...
import { PAID_ELSEWHERE } from '../lib/collect-options'
...
export function CollectionScreen() {
  ...
  // The footer's closing count, once every outcome is in: anyone who handed
  // over money (in full or part, cash or Poket) against everyone who did not.
  const paidN = MAJELIS.members.filter((m) => paidOf(s, m) > 0).length
    ...
    >
      ...
      <div className="flex flex-col gap-8 pb-16">
        ...
        {visible.map((mitra) => {
          ...
          function resultOf(): {
            ...
            if (status === 'tidak') {
              return {
                ...
                // Each refusal carries what makes it chaseable: a janji bayar,
                // or — for the two that aren't about next week — the receipt
                // she already paid with, or the date she died.
                notes: [
                  { label: 'Alasan', value: refusal?.reason ?? '—' },
                  refusal?.deathDate
                    ? { label: 'Tanggal meninggal', value: refusal.deathDate }
                    : refusal?.reason === PAID_ELSEWHERE
                      ? null
                      : { label: 'Janji bayar', value: refusal?.ptp ?? 'Tidak ada janji bayar' },
                ].filter((n): n is { label: string; value: string } => n !== null),
                proof: refusal?.reason === PAID_ELSEWHERE && Boolean(s.poketProof[mitra.id]),
              }
            }
          ...
          // Paid through another channel before this task was sent, so the
          // task came back Ditolak and the BP owes her the difference.
          const refund = s.refunds[mitra.id]
              ...
              action={
                selfPaid ? (
                  ...
                  <div className="flex flex-col gap-8">
                    ...
                    />
                    {refund ? (
                      <div className="flex items-start gap-8 rounded-8 border border-red-500 bg-red-50 p-12 text-red-500">
                        <span className="shrink-0">
                          <WarningCircle size={20} />
                        </span>
                        <span className="min-w-0 flex-1 text-14">
                          Mitra bayar via {refund.via} sebelum tugas dikirim. Kembalikan{' '}
                          <span className="font-bold">{rupiah(refund.amount)}</span> ke mitra.
                        </span>
                      </div>
                    ) : null}
                  </div>
      ...
      <StickyBar>
        ...
        {pending.length > 0 ? (
          ...
          <span className="text-center text-12 text-caption">
            {paidN} bayar • {MAJELIS.members.length - paidN} tidak bayar
          </span>
        )}
