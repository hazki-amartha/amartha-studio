...
import { OfflineSheet, SkipVisitSheet } from '../lib/visit-sheets'
import {
  attendanceComplete,
  presentCount,
  settledCount,
...
export function AttendanceScreen() {
  ...
  // The absence-reason sheet: whose card opened it, and the answer being
  // drafted. The mark only lands on "Lanjut" — "Batal" leaves the card as it
  // was, so a mis-tapped "Tidak Hadir" costs nothing.
  const [asking, setAsking] = useState<string | null>(null)
  const [draftReason, setDraftReason] = useState<string | null>(null)

  function askReason(mitraId: string) {
    setDraftReason(s.absenceReasons[mitraId] ?? null)
    setAsking(mitraId)
  }
    ...
    >
      ...
      <div className="-mx-16 -mt-16 flex flex-col gap-12 rounded-b-16 border-b border-default bg-neutral-white p-16">
        ...
        <div className="border-t border-default pt-12">
          <KumpulanCard entry={group} missing={s.addressMissing} />
        </div>
      </div>
            ...
            <MitraCard
              ...
              action={
                <div className="flex flex-col gap-8">
                  ...
                  {/* An absence carries its reason, asked in a sheet per the
                      Figma and read back on the card as a note. The pencil
                      reopens the sheet holding the answer on file. */}
                  {mark === 'tidak' && reason ? (
                    <ReasonNote label="Alasan:" value={reason} onEdit={() => askReason(mitra.id)} />
                  ) : null}
                </div>
              }
            />
      ...
      <BottomSheet
        ...
        title="Kenapa mitra tidak hadir?"
        description="Pilih alasan mitra tidak hadir."
        secondaryAction={
        ...
        primaryAction={
          <Button
            ...
            onClick={() => {
              if (asking && draftReason) store.setAbsent(asking, draftReason)
              setAsking(null)
            }}
          >
            Lanjut
      ...
      >
        <ChoiceList
          ...
          value={draftReason ?? undefined}
          onPick={setDraftReason}
        />
      </BottomSheet>

      <OfflineSheet open={s.offline} onRetry={() => store.setOffline(false)} />
