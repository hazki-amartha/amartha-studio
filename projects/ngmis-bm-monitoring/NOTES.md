...
- **The targets are monthly, the page reports a week.** The count target (20 mitra baru / bulan) is paced by `WEEKS_IN_MONTH` before a weekly figure is judged against it; the cards still name the monthly number, because that is what the business set.
- **Kurang dari target is banded, not pass/fail.** Every BP is short of a monthly target mid-month, so a red row for all of them says nothing: 40%+ of the target still missing is red, 20%+ orange, under that green.
