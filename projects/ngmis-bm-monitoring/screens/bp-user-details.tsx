...
// User details — reached from Setor tunai's "BP mangkir" action. A left
// profile card (identity, status, personal + org info) beside a right
// Activities log. The "Update" link by the status opens a dialog to change
// it. Which BP this is comes from the module store the table sets before
// navigating; the rest of the profile is representative fixed data.
//
// Ported from ngmis-cash-outstanding's fo-user-management screen (§1).
// BmShell/Panel/Select/PageHeading are our own shared components; the source
// had no back control (it lived one level under Settings), so a back button
// is added here, matching every other screen this project navigates to.

import { useEffect, useState, type ReactNode } from 'react'
import { useFlow } from '@/platform/runtime'
import { Badge, Button, Modal } from '@/design-system/components'
import { ArrowLeft, ChevronLeft, ChevronRight } from '@/design-system/icons'
import { BmShell } from '../lib/shell'
import { Panel, Select, PageHeading } from '../lib/ui'
import { useSelectedBp } from '../lib/cash-store'
...
const ACTIVITIES = [
  { title: 'Ditandai mangkir', by: 'Anda', at: '26 JUN 2026, 22:49 WIB' },
  { title: 'Status update: Active to Inactive', by: 'Andreas Mulyo', at: '06 FEB 2026, 12:09 WIB' },
  { title: 'Account creation', by: 'Robert Peterson', at: '14 APR 2024, 09:00 WIB' },
]

export function BpUserDetailsScreen() {
  const flow = useFlow()
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-start gap-16">
        <ProfileCard name={name} statusId={statusId} onUpdateStatus={() => setEditingStatus(true)} />
        <ActivitiesCard />
      </div>
...
function ProfileCard({
  ...
  return (
    <div className="shrink-0 overflow-hidden rounded-16 border border-default bg-neutral-white" style={{ width: CARD_W }}>
      <div className="h-64 bg-gradient-to-r from-neutral-900 to-primary-600" />
      <div className="flex flex-col gap-16 px-16 pb-16">
        <span className="-mt-32 flex size-64 items-center justify-center rounded-full border-4 border-neutral-white bg-green-500 text-24 font-bold text-neutral-white">
          {name.charAt(0)}
        </span>
        ...
        <div className="flex flex-col gap-2">
          ...
          <span className="flex items-center gap-8 text-14 text-default">
            ...
            <button type="button" className="text-14 font-regular text-link underline active:opacity-70">
              View ↗
            </button>
          </span>
        </div>
