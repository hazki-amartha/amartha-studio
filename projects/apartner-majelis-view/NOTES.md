...
- `PenagihanCard` — the collection record on the mitra page, under `AngsuranCard` and only for a mitra with attempts on file (every mitra in DPD; nobody who is lancar). Headline is the SENTENCE a BP says at the door — "Sudah 4 kali ditagih dalam 34 hari" — over the same attempts counted by channel; then the janji bayar, tinted, as the one row facing forward (drawn plainly as "Belum ada janji bayar" when there is none, because four visits and nothing committed is itself the finding); then the attempts, newest first, capped at three behind a "Lihat semua" expand. Every entry names WHO made it, which is the whole reason it is a list and not a counter: a mitra handed over to another BP otherwise arrives with a DPD number and no story. Data and card live together in `penagihan.tsx`, authored per mitra id — a few representative attempts, not a log.
