...
- `WaSendSheet` — every WhatsApp message this app sends, in one sheet: the recipient as a chip, the sentence already written as a read-back, one send button. It was the kumpulan reminder's sheet (`majelis.tsx` now renders it too), and the same shape answered the two Bukti pages — the majelis visit sends the group its payment recap, the home visit sends the mitra her receipt. The message is never a text field: everything in it is derived from what she just recorded, so an editable box invites an edit she didn't come for. The home visit's receipt used to be a whole PAGE (`home-wa-confirm`) with a textarea on it; that screen is gone, and with it a screen standing between a finished visit and the schedule.
