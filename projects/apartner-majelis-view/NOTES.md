...
- `KumpulanSheet` — "Menuju Lokasi Kumpulan": the kumpulan address as a maps route plus the Ketua Majelis' contact card, in one sheet. Opens twice from two directions — from the schedule when a majelis task is tapped (CTA "Mulai Kumpulan", which is what actually starts the visit) and from the `link` slot of all three visit stages' top bars (CTA replaced by "Tutup"). Lives in `kumpulan-sheet.tsx` rather than `ui.tsx` because it reads `profile.ts`, which `ui.tsx` must not depend on.
