...
- `ResultRow` — what a stage card says once its outcome is RECORDED, as opposed to `ActionRow`, which is right while the card still asks for something. The figure gets its own line at reading size with its status badge beside it, "Ubah" sits at the edge as a link, and anything further — what is still short, the reason, the janji bayar — drops to a SECOND row under a rule instead of being crushed into the first. The second row only renders when there is something to put in it, so a clean "Lunas" stays one row tall. Used on the collection stage.
