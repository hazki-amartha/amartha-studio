...
'Every row wears its kind as a short code — MV, HV, Sos, FU — the same shorthand she and her BM already speak. The time sits on the address line, in front of the pin — when and where are the same fact in two parts, and reading them together is how a stop gets planned. It used to run down a gutter outside as a clock rail, which was the last thing on this screen still arguing the day happens in order; the title now has the first line to itself, which is the only line she scans to find a row. The status sits at the right edge above the chevron, so every card reads the same way whatever its title length: what and where on the left, where it stands on the right.',
...
'No cap on settlements. Capping the count made the app hold an opinion about how often a BP should be allowed to put cash down, which is the opposite of what the feature is for — the risk being managed is money on a motorbike, and every handover reduces it. What the count decides now is the FEE: the first three are free, and the settlement page says so. A cost is a reason to think; a lock is a reason to keep carrying cash you wanted to be rid of.',
        'OLD RULE, kept for the record: three settlements a day, all three from this widget. The first two are hers to time — that is the point of putting cash down before the evening. The THIRD is the last one there is, so it stays shut until every majelis and doorstep is finished: spend it at 14.00 and the afternoon’s money has nowhere to go. While it is held the widget states the amount and says WHY it is shut, which is the difference between a rule and a bug. Each settlement gets its own VA, because a virtual account is what the branch reconciles against and three transfers to one number are three deposits nobody can tell apart at the other end.',
...
'Cash settles by the RUPIAH, not by the task. What is outstanding is everything banked minus everything handed over, so a short handover leaves a remainder and the widget comes straight back with it — the breakdown even attributes it to the pelayanan it came from, with the covered part drained off. It used to mark whole tasks settled, which meant a BP who confirmed Rp3.500.000 against a Rp3.700.000 ledger watched the missing Rp200.000 disappear from a page whose whole job is to say what she is carrying.',
        'OLD NOTE: a settlement takes EVERYTHING outstanding; there is no amount to choose. Partial handovers would need the app to hold an opinion about which rupiah in her bag belongs to which pelayanan, which it cannot check and she cannot separate — and a BP free to pick the number is a BP who can be asked why she picked it. What she chooses is when.',
