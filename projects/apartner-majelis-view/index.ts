...
'Every row wears its kind as a short code — MV, HV, Sos, FU — the same shorthand she and her BM already speak. The time sits inside the card in front of the title; it used to run down a gutter outside as a clock rail, which was the last thing on this screen still arguing the day happens in order. The status sits at the right edge above the chevron, so every card reads the same way whatever its title length: what and where on the left, where it stands on the right.',
