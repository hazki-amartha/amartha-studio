  ...
  screens: [
    {
      ...
      notes: [
        ...
        'Two filters, for the two questions a day gets asked once it stops running in order: “what home visits do I still have” and “what have I not sent”. Filtering replaces the whole agenda with one flat list — Sekarang/Berikutnya/Selesai is a shape built around WHEN, and a BP filtering by type has stopped asking that. The status badge appears only in that flat list; on the normal agenda the section a row sits in already says where it stands.',
        'Every row carries how far it is, because the order of a day is decided by geography as much as by the clock — two stops in one kampung get done together whatever their slots say. “Kemungkinan bayar tinggi” is only ever on a home visit: a majelis is 22 women with 22 answers, so one propensity flag on a group describes nobody in it. It stays a small label rather than a headline, because a BP who reads a prediction as a promise and finds an empty house twice stops believing the next one.',
