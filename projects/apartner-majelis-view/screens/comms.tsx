...
// The inbox, reached from the one icon in the schedule header. Ported from
// `apartner-homepage-ia`, where the same list sat behind Home's banner
// carousel — this direction has no carousel, so the list IS the surface.
...
export function CommsScreen() {
  ...
  const unread = unreadComms(s)

  const tagOpts = [
    { label: 'Semua tipe', value: null as string | null },
    ...COMMS_TAGS.map((t) => ({ label: t, value: t as string | null })),
  ]
  ...
  return (
    <Screen topBar={<NavigationHeader title="Informasi & Program" onBack={flow.back} />}>
      ...
      <p className="text-12 text-caption">
        {filtered
          ? `${list.length} dari ${s.comms.length} informasi`
          : `${s.comms.length} informasi · ${unread} belum dibaca`}
      </p>
            ...
            <li key={c.id}>
              ...
              >
                <Card>
                  ...
                  <p
                    className={`mt-8 text-14 font-bold ${c.read ? 'text-neutral-700' : 'text-default'}`}
                  >
                    {c.title}
                  </p>
