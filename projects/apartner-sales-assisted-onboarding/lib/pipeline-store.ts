...
export const pipelineStore = {
  ...
  saveAssistedProgress(id: string, completed: string[], whenLabel: string, note?: string) {
    completeTask(id, (lead) => ({
      ...
      log: appendLog(lead, {
        via: 'manual',
        status: lead.status,
        system: `Aplikasi assisted disimpan (${completed.length}/${APPLICATION_SECTIONS.length} bagian)`,
        note: note?.trim() || undefined,
      }),
    }))
  },
  ...
  // The onboarding flow the reference draws: after the majelis step the BP picks
  // a mode, the survey is created, and the lead walks the Leads list left to
  // right — Survey ongoing → submitted → approved — staying visible the whole way
  // (she is on both the Leads list and the Mitra list), rather than dropping off
  // the moment a survey exists.

  /**
   * Begin onboarding in the chosen mode — the survey is created. `self` invites
  ...
  beginOnboarding(id: string, mode: SurveyMode | undefined, majelis?: MajelisAssignment) {
    completeTask(id, (lead) => ({
      ...
      kumpulanStage: undefined,
      // A fresh survey supersedes the old self/assisted progress flags.
      selfServiceStarted: mode === 'self',
      assistedStarted: mode === 'assisted',
      assistedDone: [],
      lastResult: { kind: mode === 'self' ? 'self-service' : 'assisted', date: dateFromToday(0) },
      log: appendLog(lead, {
        via: 'manual',
        status: 'survey-created',
        system:
  ...
  /**
   * Take a stalled self-service survey over — the BP fills it in herself. Just
   * flips the mode; her status (Survey ongoing) is unchanged, so she stays in the
   * same column, now marked Assisted.
   */
  takeoverAssisted(id: string) {
    patchLead(id, (lead) => ({
      surveyMode: 'assisted' as const,
      selfServiceStarted: false,
      assistedStarted: true,
      log: appendLog(lead, {
        via: 'manual',
        status: lead.status,
        system: 'Application takeover — dilanjutkan assisted oleh BP',
      }),
    }))
  },

  /**
   * The survey is submitted — self-service form filed on AFin, or the assisted
   * checklist completed. She moves to Survey submitted; the system takes over
   * (KYC → underwriting).
   */
  submitSurvey(id: string) {
    patchLead(id, (lead) => ({
      ...
      log: appendLog(lead, {
        via: 'system',
        status: 'survey-submitted',
        system: 'Survey masuk — proses KYC & underwriting',
      }),
    }))
  },
  ...
  approveSurvey(id: string) {
    patchLead(id, (lead) => ({
      ...
      log: appendLog(lead, {
        via: 'system',
        status: 'approved',
        system: 'Lolos underwriting — menunggu pencairan',
      }),
    }))
  },
}
