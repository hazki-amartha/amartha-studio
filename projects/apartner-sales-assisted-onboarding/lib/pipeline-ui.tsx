 ...
 * the survey gets filled — the calon mitra doing it herself on AFin
 * (self-service), or the BP filling it in beside her (assisted). Both paths start
 * a survey; assisted opens the checklist, self-service confirms the invite. Shared
 * so the existing-majelis and new-majelis branches draw the identical sheet.
 */
export function OnboardingModeSheet({
  open,
  onClose,
  onPick,
}: {
  open: boolean
  onClose: () => void
  onPick: (mode: SurveyMode) => void
}) {
  const options: { mode: SurveyMode; title: string; description: string }[] = [
    {
      mode: 'assisted',
      title: 'Assisted',
      description: 'BP memandu dan mengisi survey bersama calon mitra di APartner.',
    },
    {
      mode: 'self',
      title: 'Self Serve on AFIN',
      description: 'Calon mitra mengisi survey sendiri di aplikasi AFIN.',
    },
  ]
  return (
    <BottomSheet
      ...
      description="Minta feedback calon mitra di tahap ini, lalu pilih cara pengisian survey."
    >
