...
/**
 * The past steps on the follow-up stepper, oldest first — the lead's real
 * history. Uses a curated `contextHistory` when the seed provides one; otherwise
 * builds it from her `log`, one step per recorded contact so the whole
 * progression shows (created → contacted → survey created/submitted/approved),
 * not just the first and last touch. A system message is the step's title (the
 * platform's own wording); a plain contact reads as its status, with the BP's
 * note quoted beneath.
 */
...
// --- Leads-list tabs (the updated Sales page) ------------------------------
//
// Phase 2 splits the Sales page into a Leads list and a POI-visit list, and the
// Leads list is filtered by where each lead sits in the onboarding funnel — the
// four columns of the reference flow. A lead stays on the Leads list the whole
// way through (she is "visible in both Mitra List and Leads List"), moving from
// one tab to the next as her survey is created, submitted and approved.

// The Leads board is a vertical stack of sections, most-progressed at the top so
 ...
 * Null before a survey exists (a plain follow-up lead has no mode yet).
 */
export function onboardingLine(lead: PipelineLead): string | null {
  if (!lead.surveyMode) return null
  if (lead.status === 'new' || lead.status === 'interested') return null
  return `${SURVEY_MODE_LABEL[lead.surveyMode]} onboarding`
}
...
/** Like `dateFromToday`, prefixed with the weekday — "Sel, 21 Jul 2026" — for
 *  the date pill on the Sales board cards. */
export function dateWithWeekday(days: number): string {
  const d = new Date(TODAY)
  d.setDate(d.getDate() + days)
...
export const SEED_PIPELINE: PipelineLead[] = [
  ...
  },
  {
    ...
    status: 'interested',
    ageDays: 6,
    agenda: { day: 'upcoming', kind: 'Follow up', when: 'Lusa', order: 6, dueDays: 2 },
  ...
  // Reactivation — an ex-mitra whose cool-off comes due today.
  {
    id: 'p13',
    name: 'Enok Suminar',
    phone: '0821-9087-3321',
    source: 'referral',
    referredBy: 'Ibu Rina (Majelis Mawar)',
    fo: 'Nurhayati',
    photo: true,
  ...
  {
    id: 'p15',
    name: 'Cicih Kurnaesih',
    phone: '0813-5521-8890',
    source: 'poi',
    poi: 'Pasar Ciseeng',
    referredBy: '',
    fo: 'Nurhayati',
    ...
    disburseDate: '',
    log: [
      { at: '18 Juli', via: 'poi', status: 'interested' },
      { at: '20 Juli', via: 'telepon', status: 'interested', note: 'Tertarik produk Modal, siap gabung majelis' },
      { at: '21 Juli', via: 'manual', status: 'interested', system: 'Aplikasi untuk majelis existing (Majelis Seruni)' },
    ],
  },
]
