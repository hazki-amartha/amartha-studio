...
/**
 * Whether a lead belongs on the Phase-2 Leads list. Unlike `inSalesFunnel`, the
 * Leads list keeps her through the whole onboarding funnel — Survey ongoing,
 * submitted and approved all still show here, in their own tab, so the BP can
 * watch the funnel drain (the reference note: "visible in both Mitra List and
 * Leads List"). Only a lead who moved to a new-majelis sosialisasi task leaves.
 */
...
/** The date as two pills — the calendar date, then how it reads relative to
 *  today. The relative pill turns orange once a follow-up is two+ days overdue. */
function relativeDue(d: number): { text: string; late: boolean } {
  if (d === 0) return { text: 'Hari ini', late: false }
  if (d < 0) return { text: d === -1 ? 'Kemarin' : `${-d} hari yang lalu`, late: -d >= 2 }
  return { text: d === 1 ? 'Besok' : `${d} hari lagi`, late: false }
}

function DatePills({ agenda, dateOnly }: { agenda?: Agenda; dateOnly?: boolean }) {
  const d = agendaDueDays(agenda)
  const rel = relativeDue(d)
  return (
    <div className="flex flex-wrap items-center gap-8">
      <span className="rounded-full border border-default px-8 py-2 text-12 text-default">
        {dateWithWeekday(d)}
      </span>
      {dateOnly ? null : (
        <span
          className={`rounded-full border px-8 py-2 text-12 ${
            rel.late ? 'border-orange-200 bg-orange-50 text-orange-500' : 'border-default text-default'
          }`}
        >
          {rel.text}
        </span>
...
/**
 * The Sales-board lead card (Phase 2 sectioned board): two date pills, name,
 * source, a pinned location, and — once a survey exists — the onboarding mode.
...
/**
 * A POI visit as a board row — the same shape as `LeadBoardCard` (date pills,
 * title, pinned location), so the POI-visit section reads consistently with the
 * lead sections around it. Rendered inside a section panel, so it takes a
 * `divider` for the hairline above every row but the first.
 */
export function PoiBoardCard({
  event,
  completed,
  onOpen,
  divider,
  ...
  return (
    ...
    >
      {noSchedule ? (
        <div className="flex">
          <span className="rounded-full border border-default px-8 py-2 text-12 text-caption">
            Belum ada jadwal
          </span>
        </div>
      ) : (
        <DatePills agenda={event.agenda} />
      )}
      <div className="flex min-w-0 flex-col gap-2">
        <span className="truncate text-16 font-bold text-default">{event.title}</span>
        <span className="flex items-center gap-4 text-12 text-caption">
