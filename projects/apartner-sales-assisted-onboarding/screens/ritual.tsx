...
// Ritual explanation — the third box of the assisted survey. Three points the BP
// runs through with the majelis, each a checkbox. Progress is saved to the survey
// store, so the survey page reflects how many are done; "Selesai" returns.
...
export function RitualScreen() {
  ...
  if (!lead) {
    return (
      <AppScreen topBar={<NavigationHeader title="Ritual explanation" onBack={() => flow.go('application')} />}>
  ...
  return (
    <AppScreen
      topBar={<NavigationHeader title="Ritual explanation" onBack={() => flow.go('application')} />}
    >
      ...
      <StickyBar>
        <Button size="lg" className="w-full" onClick={() => flow.go('application')}>
          Selesai
        </Button>
      </StickyBar>
    </AppScreen>
