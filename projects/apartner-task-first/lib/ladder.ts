...
//
// What this is FOR decides everything about its shape. The ladder is not a
// dashboard of her standing and it is not a top-up pitch: it is a BRIEFING. A BP
// arrives at a door with one hard argument — "keep paying on time and your modal
// grows" — and today she has to make it from memory, with numbers she only half
// remembers. So this module's real output is not the rungs, it is `script`: the
// sentence she says, with the right amount and the right number of weeks already
// in it.
//
// Which is also why the blocked case is the important one. Every mitra on this
// prototype's roster who is worth opening is behind, and a woman 34 days down
// must not be shown a cheerful progress bar. When there are arrears the ladder
// reports `tertahan`, states what it costs to release it (the whole tunggakan,
// not this week's angsuran), and the script changes to match. The blocked
// version is the more useful one: it turns the ladder from a cross-sell surface
// into leverage for collection, which is the work this direction cares about.
//
// Everything is derived from the mitra's existing record (`profileOf`, `due`,
// `dpd`), so a mitra has the same ladder every time she is opened, from any
// screen — the same contract the rest of the record keeps.
...
export interface Rung {
  id: string
  /** Month of the cycle this rung falls on. */
  month: number
  /** "3 Bulan" — the rail's heading. */
  title: string
  /** "TOP UP" / "PELUNASAN DINI" / "LIMIT NAIK". */
  badge: string
  kind: RungKind
...
export interface Ladder {
  ...
  current: Rung | null
  /** Weeks unpaid. 0 when she is current. */
  missedWeeks: number
  /** What it costs to release the ladder: every unpaid week, not just this one. */
  arrears: number
...
// --- Dates -----------------------------------------------------------------
// A rung she has already cleared is worth dating — "Selesai pada 18 Agu 2024" is
// what makes the argument concrete rather than hypothetical. The prototype's
// today is pinned so these dates stay stable across sessions instead of drifting
// with the real clock.

const TODAY = new Date(2025, 6, 28)
...
export function ladderOf(mitra: Mitra): Ladder {
  ...
  const plafond = mitra.due * profile.cycle
  const monthsIn = Math.floor(profile.week / 4)

  const missedWeeks = Math.ceil(mitra.dpd / 7)
  const arrears = missedWeeks * mitra.due
  const status: Ladder['status'] = missedWeeks > 0 ? 'tertahan' : 'berjalan'
  ...
  const rungs: Rung[] = MILESTONES.map((m, i) => {
    ...
    if (currentIndex === -1 || i < currentIndex) {
      return {
        ...base,
        state: 'tercapai',
        detail: `Selesai pada ${weeksAgo(profile.week - m.month * 4)}`,
        progress: null,
      }
    }
    ...
    // The rung she is on. Progress spans THIS rung, not the whole cycle — "8
    // minggu tersisa" is a number she can hold in her head; "week 36 of 48" is
    // one she has to do arithmetic on.
    const prevMonth = i === 0 ? 0 : MILESTONES[i - 1].month
    const weeksTotal = (m.month - prevMonth) * 4
    const weeksDone = Math.min(weeksTotal, Math.max(0, profile.week - prevMonth * 4))
  ...
  // at month 9 is climbing toward "pelunasan dini", and telling a woman who
  // cannot pay that clearing her arrears earns her the right to pay EARLY is not
  // an argument — it is a joke at her expense. So the pitch skips ahead to the
  // next rung that pays her in modal.
  const reward =
 ...
 * is exactly where a real argument collapses into "ada program top up, Bu". One
 * sentence with the real numbers already in it is the whole value of a briefing.
 */
function scriptFor(
  first: string,
  ...
  if (status === 'tertahan') {
    // One missed week is the same money as "this week's angsuran", so naming a
    // separate tunggakan figure there would be false precision.
    const settle =
      arrears === 0 ? 'Kalau angsuran ini lunas' : `Kalau tunggakan ${rupiah(arrears)} ini lunas`
    return `Bu ${first}, jalur naik modal Ibu sedang tertahan. ${settle}, jalurnya jalan lagi — di ${reward.month} bulan ${prize}.`
  }
